...
Default mode is Chinese pinyin (`zh-CN`, `zh`). The settings UI supports Chinese/English mode switching, full-width/half-width punctuation mode, skin colors, candidate size, user wordbook import/export/delete, dictionary update source configuration, update checks, and one-click dictionary update apply.
