...
- management surfaces used by future native menus are now explicit optional exports as well: dictionary source catalog, recognizer patterns, app-rule apply, agent config apply, and user phrase/reject/pin deletion are all probed by the TSF DLL while remaining available through `ShurufaExecuteCommand`; this keeps candidate context menus, Rime panels, agent provider settings, and app-aware status UI in Go/config instead of requiring another C++ loader pass
