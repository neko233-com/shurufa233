...
Inside SmokeEdit, press `F6` to cancel any stale IME composition/candidate state, activate the shurufa233 TSF profile for the current test session, and immediately refocus the native edit control. This is intended for local validation and does not change the Windows default input method. Press `F5` to cancel active composition/candidate state, clear the test buffer, and reset metrics.
