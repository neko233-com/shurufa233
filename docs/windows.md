...
- Rime-style slash symbol prefixes such as `/fs` and `/xh` are preserved by the Go core, daemon IPC, CLI, C ABI, and Windows TSF. In Chinese mode, pressing an unshifted `/` from an empty composition starts slash-prefixed symbol composition, lookup uses imported Rime symbol codes, and ordinary word candidates are filtered out. Shift+`/` still follows the configured question-mark punctuation path, and ASCII mode continues to pass literal slash through.
...
- `key-event-json` / `ShurufaKeyEventJSON` is preloaded by the Windows TSF DLL as a higher-level keyboard event API for future native migration; it accepts key labels, printable characters, modifiers, selected candidate index, page window, and app context, then returns `handled`, `committed`, `passThrough`, refreshed state, candidate payload, optional recognizer decision metadata, and optional navigation deltas; the Go side already resolves semicolon/quote quick select, selection movement, configured paging keys, full/half punctuation, Rime punctuation shape overrides, literal URL/email punctuation, and candidate-first punctuation commits so WeChat/Rime-like key behavior can move into Go/config before another C++ rebuild is needed
- management surfaces used by future native menus are now explicit optional exports as well: catalog/symbol lookup, schema presets, skin presets, Rime custom import, switches, app-rule resolve/apply, profile import/export, dictionary sources/manifests, agent config/apply/compose, sync import/export, and user score/phrase/reject/pin import/delete are all probed by the TSF DLL while remaining available through `ShurufaExecuteCommand`; this keeps candidate context menus, Rime panels, skin pickers, agent provider settings, and app-aware status UI in Go/config instead of requiring another C++ loader pass
