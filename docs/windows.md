...
- Rime-style runtime switches are exposed through daemon `/switches`, CLI `switches` / `switch`, C ABI `ShurufaSwitchesJSON` / `ShurufaApplySwitchJSON`, and the generic `ShurufaExecuteCommand(..., "apply-switch-json", ...)`; `ascii_mode`, `ascii_punct`, `candidate_comments`, `associations`, and `vertical_candidates` map to shared config fields; legacy `simplification` imports are accepted but normalized to simplified output without changing the TSF DLL
...
- the production line is simplified-Chinese only; the legacy `script` field is normalized to `simplified` before daemon, CLI, ABI, or native TSF consume it
...
- exact dictionary readings always rank before generated text. When a long reading has no exact row, the Go core composes it from whole dictionary words on legal pinyin syllable boundaries, prefers fewer lexical pieces, and caps generated candidates at three; character-by-character beams remain the last fallback instead of crowding correct words from the first page
- space, enter, main-row or numpad number keys, semicolon outside Microsoft double pinyin, apostrophe, brackets, page up/down, home/end, `-`, and `=` operate the visible candidate page in the current Windows TSF glue; `-` is previous page and `=` is next page in the default WeChat profile. A light Shift tap during pinyin commits the literal spelling and switches to English; the apostrophe separator behavior is already available through the shared Go core and preview/IPC paths while native key mapping stays compatible with the existing candidate strip and paired quote behavior
