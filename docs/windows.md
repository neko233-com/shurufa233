...
- Rime `*.custom.yaml` patches are exposed through daemon `/rime/custom`, CLI `rime import`, C ABI `ShurufaApplyRimeCustomJSON`, and the generic `rime-custom-json` command; common `schema_list`, `menu/page_size`, `speller/algebra`, `switches`, `style`, `punctuator`, `recognizer`, `key_binder`, and `ascii_composer` fields are mapped to shared config without teaching the TSF DLL YAML parsing, common fuzzy `derive` spelling rules become active `fuzzyInitials` pairs, custom `punctuator/full_shape` / `half_shape` maps are read by the TSF punctuation hot path, and `recognizer/patterns` is loaded into shared config plus TSF's reserved config cache
...
- application-aware behavior rules are exposed through daemon `/app-rules` and `/app-context/resolve`, CLI `app-rules` / `app-context resolve`, C ABI `ShurufaAppRulesJSON` / `ShurufaResolveAppContextJSON`, and the generic `resolve-app-context-json` command; built-in rules make password fields, terminals, and game/esports contexts prefer English/half-width/no-candidate behavior without taking over the Windows default input method
...
- Rime-style recognizer patterns are stored in `recognizerPatterns`; default email, URL, uppercase, and reverse-lookup rules keep literal buffers intact or route backtick-prefixed lookup through the Go candidate engine while more complex custom regex rules remain available through the ABI/config surface for later native handling
...
- `key-event-json` / `ShurufaKeyEventJSON` is preloaded by the Windows TSF DLL as a higher-level keyboard event API for future native migration; it accepts key labels, printable characters, modifiers, selected candidate index, page window, and app context, then returns `handled`, `committed`, `passThrough`, refreshed state, candidate payload, and optional navigation deltas; the Go side already resolves semicolon/quote quick select, selection movement, configured paging keys, full/half punctuation, Rime punctuation shape overrides, and candidate-first punctuation commits so WeChat/Rime-like key behavior can move into Go/config before another C++ rebuild is needed
- management surfaces used by future native menus are now explicit optional exports as well: dictionary source catalog, recognizer patterns, app-rule apply, and user phrase/reject/pin deletion are all probed by the TSF DLL while remaining available through `ShurufaExecuteCommand`; this keeps candidate context menus, Rime panels, and app-aware status UI in Go/config instead of requiring another C++ loader pass
