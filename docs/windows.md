...
- Rime `*.custom.yaml` patches are exposed through daemon `/rime/custom`, CLI `rime import`, C ABI `ShurufaApplyRimeCustomJSON`, and the generic `rime-custom-json` command; common `schema_list`, `menu/page_size`, `speller/algebra`, `switches`, `style`, `punctuator`, `key_binder`, and `ascii_composer` fields are mapped to shared config without teaching the TSF DLL YAML parsing, and common fuzzy `derive` spelling rules become active `fuzzyInitials` pairs
