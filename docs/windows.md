...
- management surfaces used by future native menus are now explicit optional exports as well: catalog/symbol lookup, schema presets, skin presets, Rime custom import, switches, app-rule resolve/apply, profile import/export, dictionary sources/manifests/update check/update apply, agent config/apply/compose, sync import/export, and user score/phrase/reject/pin import/delete are all probed by the TSF DLL while remaining available through `ShurufaExecuteCommand`; this keeps candidate context menus, Rime panels, skin pickers, dictionary update panels, agent provider settings, and app-aware status UI in Go/config instead of requiring another C++ loader pass
