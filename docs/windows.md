...
- `candidate-action` / `ShurufaCandidateAction` reserve page, select, candidate hiding, first/last-character, and future Rime-style candidate events behind that JSON command bus, keeping the hot TSF layer thin for machines that only consume packaged builds
